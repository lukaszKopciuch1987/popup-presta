...
abstract class PopupValidatorCore
{
    protected $module;
    protected $formName;
    ...
    public function __construct($moduleObject, $formName)
    {
        if ($moduleObject instanceof Module) {
            ...
            throw new \Exception(" Incorrect module instance");
        }
        ...
        if (!file_exists($formDir .$formName.'.php')) {
            throw new \Exception(" Form '{$formName}' file not found in classes/form directory");
        } else {
            ...
            $this->validation = new PopupValidation($moduleObject);
    ...
    public function validate()
    {
        if (!$this->allowEmpty) {
            if (!$this->getData() || count($this->getData()) == 0) {
                throw new \Exception(" Form data for '{$this->formName}' is not provided");
            }
        }
    ...
    public function getField($field)
    {
        if (array_key_exists($field, $this->data)) {
            ...
            throw new \Exception("Field not found in getField() method");
        }
    }
}
